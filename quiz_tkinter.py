...
ligas = {
    ...
    "Primera Nacional": {
        "ID": 129,
        "URL": "https://media.api-sports.io/football/leagues/129.png"
    },
    "Primeira Liga": {
        "ID": 94,
    ...
    "MLS": {
        "ID": 866,
        "URL": "https://media.api-sports.io/football/leagues/866.png"
    }
}
...
def solicitar_nome():
    ...
    tk.Label(window, text="Por favor, insira seu nome:").grid(row=0, column=0)
    nome_entry = tk.Entry(window)
    nome_entry.grid(row=0, column=1)
    tk.Button(window, text="Confirmar", command=lambda: confirmar_nome(nome_entry)).grid(row=2, column=0, columnspan=2)
...
def mostrar_ligas():
    ...
    for liga, info in ligas.items():
        ...
        btn = tk.Button(window, image=photo, text=liga, compound="top", command=lambda id=info["ID"]: selecionar_liga(id))
        btn.image = photo  # Manter uma referência da imagem
        btn.grid(row=0, column=coluna)  # Posicionar o botão na linha 0 e na coluna atual
...
def mostrar_anos():
    ...
    label_instrucao = tk.Label(window, text="Escolha o ano do campeonato:")
    ...
    for ano in anos:
        btn_ano = tk.Button(window, text=str(ano), command=lambda a=ano: selecionar_ano(a))
        btn_ano.pack()
...
def mostrar_carregando(funcao):
    ...
    carregando_label = tk.Label(window, text="    Carregando...    Aguarde por favor!")
...
def start_quiz(category):
    ...
    # Exibir a pergunta
    question_label = tk.Label(window, text=f"{texto_quiz}")
    question_label.pack()
    ...
    for player in data['response']:
        if player['player']['name'] in random_names:
            ...
            btn = tk.Button(window, image=photo, command=lambda n=player['player']['name']: check_answer(n, correct_answer))
            btn.image = photo  # Manter uma referência da imagem
            btn.pack()
            # Adicionar o nome do jogador abaixo da foto
            name_label = tk.Label(window, text=player['player']['name'])
            name_label.pack()
